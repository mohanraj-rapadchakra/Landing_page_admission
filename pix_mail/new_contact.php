...
//			}
	...
	function sendAWeber($mailSubscribe, $the_list, $merge_vars=NULL, $language, $lang){
		if(defined('AW_AUTHCODE') && $the_list){
            if(!$merge_vars){
                $merge_vars=[];
            }
...
//			}
            ...
            curl_setopt_array($curl, array(
                CURLOPT_URL => "https://api.mailerlite.com/api/v2/groups/".MailerLite_LIST_ID."/subscribers",
            ...
            if ($err) {
                $output = json_encode(array('type'=>'error', 'text' => 'Error:'. $err));
            } else {
                $output = json_encode(array('type'=>'message', 'text' => $lang['EN']['subscription'] ));
            }
